...
module YtSubFetch
  class YouTubeSubtitleFetch
    ...
    def validate_youtube_url(url)
      ...
      unless url && url.match?(youtube_regex)
    ...
    def fetch_subtitles
      ...
      # 保存原始響應以檢查
      File.open('youtube_response.html', 'w') { |f| f.write(response.body) } if @options[:debug]
        ...
        matches.each do |match|
          begin
            ...
            clean_json = match[0].gsub(/\\(["\\\/bfnrt])/, '\1')
                                 .gsub(/\\u([0-9a-fA-F]{4})/) { |m| [m[2..-1].to_i(16)].pack('U') }
            ...
            caption_tracks.each do |track|
              ...
              subtitles << {
                ...
                name: (track['name']['simpleText'] rescue track['name']),
                url: track['baseUrl']
              }
            end
